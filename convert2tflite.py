...
from netOrigin import deepfloorplanModel
...
print("Colab System Version: ", platform.platform())
...
def parse_args(args: List[str]) -> EasyDict:
    parsed_args = EasyDict()

    parsed_args.tfmodel = "subclass"
    parsed_args.modeldir = "model/store"
    parsed_args.tflitedir = "model/store/model.tflite"
    parsed_args.quantize = False
    parsed_args.compress_mode = "quantization"
    parsed_args.loadmethod = "log"
    parsed_args.feature_channels = [256, 128, 64, 32]
    parsed_args.backbone = "vgg16"
    parsed_args.feature_names = ["block1_pool", "block2_pool", "block3_pool", "block4_pool", "block5_pool"]
    parsed_args.tomlfile = None
...
def prune(config: argparse.Namespace):
    ...
    for _ in range(epochs):
        ...
        for data in tqdm(list(dataset.batch(batches))):
            ...
            _, _, loss_value, _, _ = train.train_step(
...
!python -m convert2tflite --loadmethod 'pb'
